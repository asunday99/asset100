...
def _render_trade_calendar(df_rec: pd.DataFrame):
    ...
    tbl  = '<table style="width:100%;table-layout:fixed;border-collapse:collapse;border:1px solid #C8D4E5;margin-top:8px;">'
    tbl += "<tr>"
    for day_name in ["일", "월", "화", "수", "목", "금", "토"]:
        tbl += f'<th style="background-color:#000000;color:#a0a0a0;padding:6px 2px;border:1px solid #C8D4E5;text-align:center;font-size:12px;">{day_name}</th>'
    tbl += "</tr>"
    for week in weeks:
        tbl += "<tr>"
        for d in week:
            day_text   = str(d.day) if d.month == _disp_month else " "
            style_date = "height:24px;background:linear-gradient(180deg, rgba(45, 55, 75, 0.5) 0%, rgba(13, 16, 22, 0.9) 100%);color:#a0a0a0;font-weight:bold;text-align:left;padding:3px 4px;border:1px solid #C8D4E5;font-size:11px;"
        ...
        for d in week:
            if d.month == _disp_month and d in daily_pnl_dict:
                ...
                val_color     = "white"
                formatted_val = "&nbsp;"
            style_data = f"height:60px;background-color:#000000;color:{val_color};border:1px solid #C8D4E5;text-align:center;vertical-align:middle;padding:3px;font-size:12px;font-weight:bold;"
