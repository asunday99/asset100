        ...
        try:
            if '^TNX' in closes and '^IRX' in closes:
                ...
                if len(tnx_c) >= 2 and len(irx_c) >= 2:
                    ...
                    changes['[장단기 금리차]'] = float(spread_today - spread_yest)
        except: pass
        ...
        try:
            if 'XLF' in closes and 'QQQ' in closes:
                ...
                if len(xlf_c) >= 2 and len(qqq_c) >= 2:
                    today_ratio = xlf_c.iloc[-1] / qqq_c.iloc[-1]
                    yest_ratio = xlf_c.iloc[-2] / qqq_c.iloc[-2]
                    changes['[XLF-QQQ괴리율]'] = float(today_ratio - yest_ratio)
        except: pass

        # 8. ADX 추세강도 (QQQ 기준)
        try:
            if 'QQQ' in highs and 'QQQ' in lows and 'QQQ' in closes:
                q_h = highs['QQQ'].dropna()
                q_l = lows['QQQ'].dropna()
                q_c = closes['QQQ'].dropna()
                if len(q_c) >= 28:
                    up_move = q_h.diff()
                    down_move = -q_l.diff()
                    pos_dm = np.where((up_move > down_move) & (up_move > 0), up_move, 0.0)
                    neg_dm = np.where((down_move > up_move) & (down_move > 0), down_move, 0.0)
                    tr = np.maximum(q_h - q_l, np.maximum(abs(q_h - q_c.shift(1)), abs(q_l - q_c.shift(1))))
                    atr = pd.Series(tr).rolling(14).mean()
                    pos_di = 100 * (pd.Series(pos_dm, index=q_h.index).rolling(14).mean() / atr)
                    neg_di = 100 * (pd.Series(neg_dm, index=q_h.index).rolling(14).mean() / atr)
